...
const setDefaultOptions = (options) => (Object.assign({}, options, { javascript: !hasAnyLintingSet(options) || options.javascript, typescript: !hasAnyLintingSet(options) || options.typescript, pattern: options.pattern || './' }));
...
const lint = (options) => {
    const result = {};
    const { javascript, typescript, includeJsx, pattern, configFile, fix, spec, sass, } = setDefaultOptions(options);
    if (typescript || javascript) {
        ...
        const baseConfig = Object.assign({}, (includeJsx ? { parserOptions: { ecmaFeatures: { jsx: true } } } : {}), require(configFile || `${selfPath}/config/recommended`));
